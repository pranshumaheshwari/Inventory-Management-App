...
app.post("/report/FG_Name", async function (req, res) {
	...
	var currentStock,
		...
		finished_goods
	var q =
		"SELECT * FROM finished_goods WHERE code ='" + finished_good_code + "'"
	await selectQuery(q)
		.then((Finished_good) => {
		...
		.then(async (_) => {
			q =
				"SELECT SUM(quantity) AS sum FROM production WHERE date >='" +
				from +
				"' AND date <='" +
				to +
				"' AND FG_code ='" +
		...
		.then(async (_) => {
			q =
				"SELECT SUM(quantity) AS sum FROM dispatch WHERE date >='" +
				from +
				"' AND date <='" +
				to +
				"' AND FG_code ='" +
		...
		.then(async (_) => {
			...
			q =
				"SELECT * ,'p' AS type FROM production WHERE FG_code ='" +
				finished_good_code +
				"' AND date >='" +
				from +
				"' AND date <='" +
		...
		.then(async (_) => {
			q =
				"SELECT * FROM dispatch WHERE FG_code ='" +
				finished_good_code +
				"' AND date >='" +
				from +
				"' AND date <='" +
...
app.post("/report/date", async function (req, res) {
	...
	await selectQuery(q)
		.then((finished_goods) => {
			res.render("report_FG-date_data", {
				finished_goods: finished_goods,
				type: req.body.type,
				from: req.body.from,
				to: req.body.to
	...
	await selectQuery(q)
		...
		.then(async (_) => {
			...
			await selectQuery(q)
				.then((sum) => {
					sum = sum[0]
					if (sum.sum) totalExchange = sum.sum
					else totalExchange = 0
				})
				.catch((err) => {
					logger.error({
						error: err,
		...
		.then(async (_) => {
			q =
				"SELECT * FROM production WHERE FG_code ='" +
				finished_good_code +
				"' AND date >='" +
				from +
				"' AND date <='" +
...
app.post("/report/name", async function (req, res) {
	...
	let q = `
			SELECT PO_code, invoice_no, quantity, date, DTPL_code issuing_req_id, raw_desc issuing_quantity, 'i' type FROM input WHERE DTPL_code = "${raw[1]}" AND raw_desc = "${raw[2]}" AND date >= "${from}" AND date <= "${to}" + INTERVAL 1 DAY 
			UNION 
			SELECT req_id, RM_code, quantity, date, issuing_req_id, issuing_quantity, 'ro' type FROM requisition_output WHERE RM_code = "${raw[0]}" AND date >= "${from}" AND date <= "${to}" + INTERVAL 1 DAY 
			UNION 
			SELECT p.id, f.raw_material_code, f.quantity * p.quantity quantity, p.date, p.quantity, f.code, 'prod' type FROM production p INNER JOIN finished_goods_detail f ON p.FG_code = f.code WHERE f.raw_material_code = "${raw[0]}" AND date >= "${from}" AND date <= "${to}" + INTERVAL 1 DAY 
			UNION 
			SELECT slip_no, raw_material_code, quantity, date,'o' temp1, 'o' temp2, 'o' type FROM output WHERE raw_material_code = "${raw[0]}" AND date >= "${from}" AND date <= "${to}" + INTERVAL 1 DAY 
			ORDER BY date
		`
	let data = await selectQuery(q).catch((err) => {
		logger.error({
			error: err,
			where: `${req.method} ${req.url} ${q}`,
			time: new Date().toISOString()
	...
	let excessOnLine = await selectQuery(q)
		.then((d) => d[0].quantity)
		.catch((err) => {
			logger.error({
				error: err,
	...
	let totalInputInRange = await selectQuery(q)
		.then((d) => d[0].quantity)
		.catch((err) => {
			logger.error({
				error: err,
	...
	let totalOutputInRange = await selectQuery(q)
		.then((d) => d[0].quantity)
		.catch((err) => {
			logger.error({
				error: err,
	...
	totalOutputInRange += await selectQuery(q)
		.then((d) => d[0].quantity)
		.catch((err) => {
			logger.error({
				error: err,
	...
	let totalInputOutRange = await selectQuery(q)
		.then((d) => d[0].quantity)
		.catch((err) => {
			logger.error({
				error: err,
	...
	let totalOutputOutRange = await selectQuery(q)
		.then((d) => d[0].quantity)
		.catch((err) => {
			logger.error({
				error: err,
	...
	totalOutputOutRange += await selectQuery(q)
		.then((d) => d[0].quantity)
		.catch((err) => {
			logger.error({
				error: err,
	...
	q = `SELECT SUM(f.quantity * p.quantity) quantity FROM production p INNER JOIN finished_goods_detail f ON p.FG_code = f.code WHERE f.raw_material_code = "${raw[0]}" AND date >= "${from}" AND date <= "${to}"`
	let totalProductionOut = await selectQuery(q)
		.then((d) => d[0].quantity)
		.catch((err) => {
			logger.error({
				error: err,
	...
	q = `SELECT SUM(f.quantity * p.quantity) quantity FROM production p INNER JOIN finished_goods_detail f ON p.FG_code = f.code WHERE f.raw_material_code = "${raw[0]}" AND date > "${to}"`
	let totalProductionOutOutRange = await selectQuery(q)
		.then((d) => d[0].quantity)
		.catch((err) => {
			logger.error({
				error: err,
	...
	let openingStoreStock =
		...
		totalInputOutRange
	// TODO - check
	let openingLineStock =
		currentLineStock +
		totalProductionOut +
		totalOutputOutRange -
		totalOutputInRange -
		totalProductionOutOutRange
	res.render("reports_with_data", {
		excessOnLine,
		input_output: data,
		currentStoreStock,
		currentLineStock,
...
app.get("/report/PO/:code", async function (req, res) {
	...
	await selectQuery(q)
		.then((inputs) => {
			res.render("report_by_PO_detail", {
				PO_code: req.params.code,
				inputs: inputs
			})
		})
		.catch((err) => {
...
app.post("/report/PO", async function (req, res) {
	var q =
		'SELECT p.*,i.sum FROM PO_detail AS p LEFT OUTER JOIN (SELECT SUM(quantity) AS sum,PO_code,raw_desc FROM input WHERE PO_code = "' +
		req.body.PO.code +
		'" GROUP BY raw_desc) AS i ON p.PO_code = i.PO_code AND p.raw_desc = i.raw_desc WHERE p.PO_code = "' +
		req.body.PO.code +
		'"'
	await selectQuery(q)
		.then((PO) => {
...
app.get("/report/shortage", async function (req, res) {
	var q =
		"SELECT * FROM raw_material WHERE stock<0.25*monthly_requirement ORDER BY code"
	await selectQuery(q)
		.then((raw) => {
			res.render("report_more_less", {
				raw_materials: raw,
				type: "shortage"
			})
		})
...
app.get("/report/excess", async function (req, res) {
	var q =
		"SELECT * FROM raw_material WHERE stock>monthly_requirement ORDER BY code"
	await selectQuery(q)
		.then((raw) => {
			res.render("report_more_less", {
				raw_materials: raw,
...
app.get("/report/ScheduleTracker", async (req, res) => {
	...
	var date = d.getDate()
	var q =
		"SELECT code, category, quantity FROM finished_goods ORDER BY category"
	var finished_goods = await selectQuery(q).catch((err) => {
		logger.error({
			error: err,
			where: `${req.method} ${req.url} ${q}`,
			time: new Date().toISOString()
	...
	for (var i = 0; i < finished_goods.length; i++) {
		...
		finished_goods[i].dispatch = await selectQuery(q).catch((err) => {
			logger.error({
				error: err,
				where: `${req.method} ${req.url} ${q}`,
				time: new Date().toISOString()
...
app.get("/report/ProductionTracker", async (req, res) => {
	...
	for (var i = 0; i < finished_goods.length; i++) {
		...
		finished_goods[i].production = await selectQuery(q).catch((err) => {
			logger.error({
				error: err,
				where: `${req.method} ${req.url} ${q}`,
				time: new Date().toISOString()
...
app.post("/report/manPower", async (req, res) => {
	...
	let data = await selectQuery(
		`SELECT code, name, category, man_power, overheads FROM finished_goods ORDER BY code`
	)
	// let q = `
	...
	let attendance = []
	let numberOfDays =
		(dateTo.getTime() - dateFrom.getTime()) / (1000 * 3600 * 24) + 1
	for (let i = 0; i < numberOfDays; i++) {
		for (let r of data) {
			...
			let q = `
					...
					FROM production p
					WHERE DAY(p.date) = '${date.getDate()}' AND MONTH(p.date) = '${
				date.getMonth() + 1
			}' AND YEAR(p.date) = '${date.getFullYear()}' AND p.FG_code = "${
				r.code
			}"
					`
		...
		q = `SELECT SUM(nos) nos FROM attendance WHERE DAY(date) = '${date.getDate()}' AND MONTH(date) = '${
			date.getMonth() + 1
		}' AND YEAR(date) = '${date.getFullYear()}'`
		attendance.push(
			await selectQuery(q)
				.then((data) => data[0])
...
app.post("/report/:type", async function (req, res) {
	...
	await selectQuery(q)
		.then((data) => {
			res.render("report_inputORoutput", {
				i_o: data,
...
app.post("/report", function (req, res) {
	var by = req.body.by
	if (by === "Name") res.redirect("/report/name")
	else if (by === "PO") res.redirect("/report/PO")
	else if (by === "input") res.redirect("/report/input")
	else if (by === "output") res.redirect("/report/output")
	else if (by === "less") res.redirect("/report/shortage")
	else if (by === "more") res.redirect("/report/excess")
	else if (by === "Production") res.redirect("/report/production")
	else if (by === "Dispatch") res.redirect("/report/dispatch")
	else if (by === "Date") res.redirect("/report/date")
	else if (by === "FGName") res.redirect("/report/FG_Name")
	else if (by === "ScheduleTracker") res.redirect("/report/ScheduleTracker")
